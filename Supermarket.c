...
int		initSuperMarket(SuperMarket* pMarket, const char* fileName,
	const char* customersFileName,int howToRead)
{
	...
	if (howToRead == 0) {
		if (loadSuperMarketFromFile(pMarket, fileName, customersFileName,howToRead) == 1)
		{
	...
	{
		if (readSuperMarketFromBinaryFileCompress(pMarket, fileName, customersFileName))
		{
			printf("Supermarket successfully loaded from files Compress File\n");
			return 1;
		}
