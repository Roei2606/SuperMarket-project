...
int compressAndSaveAddressToFile(const Address* pAdd, FILE* fp,int isComp) {
	BYTE size = (BYTE)pAdd->num;

	if(fwrite(&size, sizeof(BYTE), 1, fp) != 1) 
	{
		printf("\nERROR PRINT HOUSE NUMBER TO FILE\n");
		return 0;
...
int loadAddressFromCompressFile(Address* pAdd, FILE* fp)
{
	...
	pAdd->city = readStringFromFile(fp, "error read city name\n", 1);
	pAdd->street = readStringFromFile(fp, "error read street name\n", 1);
	if (pAdd->city == NULL || pAdd->street == NULL)
	{
		free(pAdd->street);
		free(pAdd->city);
		return 0;
